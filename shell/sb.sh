...
get_latest_tag() {
    for mirror in "${MIRRORS[@]}"; do
        ...
        if command -v curl &>/dev/null; then
            tag=$(curl -k -Ls -o /dev/null -w "%{url_effective}" --connect-timeout 5 "$url" | awk -F'/' '{print $NF}')
        else
            tag=$(wget --no-check-certificate --max-redirect=0 "$url" 2>&1 | grep -i "Location:" | awk '{print $NF}' | awk -F'/' '{print $NF}')
        fi
...
download_and_install() {
    ...
    if [[ -f "./sing-box" ]]; then
        echo -e "${YELLOW}检测到当前目录下存在 sing-box 文件。${PLAIN}"
        read -r -p "是否直接安装此本地文件? [Y/n] " install_local
    ...
    if [[ -n "$current_ver" ]]; then
        ...
        if [[ "$current_ver" == "$tag_version" || "$current_ver" == "${tag_version#v}" ]]; then
             read -r -p "版本一致，是否强制重新安装? [y/N] " reinstall
             if [[ "$reinstall" != "y" && "$reinstall" != "Y" ]]; then return; fi
        else
             read -r -p "是否升级/降级? [Y/n] " update_confirm
    ...
    _log "准备下载: ${GREEN}${filename}${PLAIN}"
...
update_config() {
    ...
    if [[ -n "$DEFAULT_CONF_URL" ]]; then
        ...
        read -r -p "使用默认地址? [Y/n] " choice
        [[ "$choice" != "n" && "$choice" != "N" ]] && url="$DEFAULT_CONF_URL"
    fi

    if [[ -z "$url" ]]; then read -e -r -p "请输入配置 URL: " url; fi
    [[ -z "$url" ]] && return

    if [[ ! "$url" =~ ^https?:// ]]; then
        _err "无效的链接！需要以 http:// 或 https:// 开头。"
        return
    fi
...
uninstall_script() {
    ...
    read -r -p "是否同时卸载 Sing-box 服务 (删除程序/配置/服务)? [y/N] " del_sb
    if [[ "$del_sb" == "y" || "$del_sb" == "Y" ]]; then
        systemctl stop sing-box 2>/dev/null
...
"
        ...
        [[ "$choice" != "7" && "$choice" != "0" ]] && read -n 1 -s -r -p " 按任意键继续..."
    done
}
